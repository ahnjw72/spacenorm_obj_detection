...
# ===== Configuration =====
EBS_MOUNT="/mnt/spacenorm_data"
DOCKER_ROOT="${EBS_MOUNT}/docker"
CONTAINERD_ROOT="${EBS_MOUNT}/containerd"

DOCKER_CONFIG="/etc/docker/daemon.json"
CONTAINERD_OVERRIDE="/etc/systemd/system/containerd.service.d/override.conf"

echo "=== Docker + containerd EBS Relocation Script ==="
...
if ! mountpoint -q "$EBS_MOUNT"; then
  echo "ERROR: $EBS_MOUNT is not a mounted filesystem."
  echo "       Mount your EBS disk first."
  exit 1
fi

echo "EBS mount verified at $EBS_MOUNT"
df -h "$EBS_MOUNT"
...
echo "=== SUCCESS: Docker and containerd are now using EBS ==="
